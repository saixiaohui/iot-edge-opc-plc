...
namespace OpcPlc
{
    public static class DiagnosticsConfig
    {
        ...
        private static IDictionary<string, object> BaseDimensions = new Dictionary<string, object>
            {
        ...
        private static readonly ILogger Logger = new SerilogLoggerFactory(Program.Logger).CreateLogger("Metrics");

        private static UpDownCounter<int> PodCount = Meter.CreateUpDownCounter<int>(OPC_PLC_POD_COUNT_METRIC);
        private static UpDownCounter<int> SessionCount = Meter.CreateUpDownCounter<int>(OPC_PLC_SESSION_COUNT_METRIC);
        private static UpDownCounter<int> SubscriptionCount = Meter.CreateUpDownCounter<int>(OPC_PLC_SUBSCRIPTION_COUNT_METRIC);
        private static UpDownCounter<int> MonitoredItemCount = Meter.CreateUpDownCounter<int>(OPC_PLC_MONITORED_ITEM_COUNT_METRIC);
        private static Counter<int> PublishedCount = Meter.CreateCounter<int>(OPC_PLC_PUBLISHED_COUNT_METRIC);
        private static Counter<int> PublishedCountWithType = Meter.CreateCounter<int>(OPC_PLC_PUBLISHED_COUNT_WITH_TYPE_METRIC);
        private static Counter<int> TotalErrors = Meter.CreateCounter<int>(OPC_PLC_TOTAL_ERRORS_METRIC);
        ...
        {
            Logger.LogDebug("Increased metric: {MertricName} with value: {MetricValue} and dimensions: {Dimensions}", metricName, metricValue, dimensions.ToJson());
        }
        ...
        {
            Logger.LogDebug("Increased metric: {MetricName} with value: {MetricValue} and type: {Type} and dimensions: {Dimensions}", metricName, metricValue, metricType, dimensions.ToJson());
        }
